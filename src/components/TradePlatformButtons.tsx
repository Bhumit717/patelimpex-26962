...
const TradePlatformButtons = () => {
  return (
    <div className="mt-8 p-6 bg-accent-ink rounded-2xl  border border-transparent">
      <h3 className="text-lg font-bold text-foreground mb-4 flex items-center">
        <ExternalLink className="h-5 w-5 text-accent-ink mr-2" />
      ...
      <div className="flex flex-wrap gap-2">
        {tradePlatforms.map((platform) => (
          <a
            ...
            className="inline-flex items-center px-3 py-1.5 text-xs font-medium bg-card hover:bg-secondary text-foreground hover:text-accent-ink rounded-full border border-border hover:border-border  hover: transition-all duration-200"
          >
            {platform.name}
