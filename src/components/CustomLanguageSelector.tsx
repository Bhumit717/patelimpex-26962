        ...
        <>
            ...
            <div className="relative z-[100]">
                <button
                    ...
                    className="flex items-center justify-center w-10 h-10 rounded-full bg-card border border-border  hover:bg-secondary transition-all duration-300 active:scale-95"
                    aria-label="Select Language"
                >
                    <Languages className="h-5 w-5 text-accent-ink" />
                </button>
                ...
                {isOpen && (
                    <>
                        ...
                        <div className="absolute top-12 left-1/2 -translate-x-1/2 lg:left-auto lg:translate-x-0 lg:right-0 bg-card rounded-[20px]  border border-border overflow-hidden z-[100] w-[90vw] max-w-[320px] lg:min-w-[280px] max-h-[60vh] overflow-y-auto scrollbar-hide">
                            <div className="p-4 space-y-2">
                                {languages.map((lang) => (
                                    <button
                                        key={lang.code}
                                        onClick={() => handleLanguageSelect(lang.code, lang.name)}
                                        className={`w-full text-left px-5 py-3 text-sm rounded-xl transition-all duration-300 hover:bg-card font-graduate ${selectedLanguage === lang.name ? 'text-accent-ink font-bold bg-secondary' : 'text-muted-foreground font-medium'}`}
                                    >
                                        <div className="flex items-center justify-between">
                                            <span>{lang.nativeName}</span>
                                            {selectedLanguage === lang.name && <div className="h-2 w-2 rounded-full bg-accent-ink shadow-[0_0_10px_rgba(22,163,74,0.5)]"></div>}
                                        </div>
                                    </button>
                                ))}
                            </div>
