...
const RelatedLinks = ({ currentPage, category = "agricultural" }: RelatedLinksProps) => {
  ...
  return (
    <div className="bg-gradient-to-br /50 /50 backdrop-blur-sm p-8 rounded-2xl border border-primary/20">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-2xl font-bold text-[hsl(var(--paper))]">Related Pages</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredLinks.map((link, index) => (
          <Link
            key={index}
            to={link.url}
            className="group flex flex-col p-4 bg-[hsl(var(--ink))]/50 hover:bg-[hsl(var(--ink))]/50 rounded-lg border border-primary/10 hover:border-primary/30 transition-all duration-300"
          >
            <span className="text-xs text-primary mb-2">{link.category}</span>
            <span className="text-[hsl(var(--paper))] group-hover:text-primary transition-colors">
              {link.title}
            </span>
          </Link>
        ))}
      ...
      <div className="mt-6 pt-6 border-t border-primary/20">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Link to="/more" className="text-sm text-muted-foreground hover:text-primary transition-colors">
            More Resources →
          </Link>
