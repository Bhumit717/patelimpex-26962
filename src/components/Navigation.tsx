...
const Navigation = () => {
  ...
  return (
    <header className={`fixed w-full z-50 transition-all duration-500 ${isScrolled ? 'top-2' : 'top-0'}`} role="banner">
      <div className="container mx-auto px-4">
        <nav className={`transition-all duration-500 flex justify-between items-center rounded-full border-none shadow-xl bg-white/90 backdrop-blur-md px-6 lg:px-10 ${isScrolled ? 'h-20' : 'h-24 md:h-28'}`}>
          ...
          <div className="flex items-center space-x-6">
            ...
            <Link to="/inquiry">
              <button className="!h-12 md:!h-16 !px-8 lg:!px-10 nm-btn-green hover:scale-105 active:scale-95">
                <span className="font-bold font-graduate uppercase text-[11px] tracking-widest">Get Quote</span>
              </button>
            </Link>

            <button
              onClick={() => setIsOpen(!isOpen)}
              className="lg:hidden w-12 h-12 flex items-center justify-center bg-slate-900 text-white rounded-full shadow-lg hover:bg-slate-800 transition-colors"
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden fixed inset-x-0 top-[110px] px-4 transition-all duration-500 ease-out ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10 pointer-events-none'}`}>
        <div className="nm-card !p-8 flex flex-col space-y-4 rounded-[40px] shadow-2xl bg-white border-2 border-slate-50">
          {navItems.map((item) => (
            <Link
              key={item.name}
              to={item.href}
              onClick={() => setIsOpen(false)}
              className={`flex items-center justify-between p-5 rounded-2xl transition-all ${location.pathname === item.href ? 'bg-green-50 text-green-700' : 'hover:bg-slate-50 text-slate-700'}`}
            >
          ...
          <Link to="/inquiry" onClick={() => setIsOpen(false)} className="pt-4">
            <button className="nm-btn-dark w-full !h-20 text-xl font-black font-graduate bg-slate-900 !text-white uppercase tracking-widest rounded-[20px] shadow-2xl transition-all duration-300 border-none">
              REQUEST FREE QUOTE
            </button>
          </Link>
        </div>
