      ...
      <main className="single_product_page" style={{ paddingTop: "100px" }}>
        ...
        <section className="single_product_sec">
          <div className="trigol-container">
            {/* Breadcrumbs */}
            <div className="flex items-center flex-wrap gap-1 text-xs font-semibold mb-8 uppercase tracking-widest text-muted-foreground">
              {breadcrumbs.map((item, i) => (
                <span key={i} className="flex items-center">
                  {item.href ? (
                    <Link to={item.href} className="hover:text-accent-ink transition-colors">{item.label}</Link>
                  ) : (
