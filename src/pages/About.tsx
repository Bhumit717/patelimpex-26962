...
const About = () => {
  ...
  return (
    <div className="min-h-screen bg-card">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="reveal">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-6 h-[2px] bg-accent-ink"></div>
                <span className="text-accent-ink font-bold uppercase tracking-widest text-xs">About Us</span>
              </div>
              <h1 className="text-5xl md:text-6xl font-black text-foreground mb-12">
                Welcome to <span className="text-foreground">Patel Impex</span>
              </h1>
            ...
            {/* Right Side: Text */}
            <div className="space-y-6 text-muted-foreground leading-relaxed reveal" style={{ animationDelay: '200ms' }}>
              <p>
              ...
              <Button onClick={() => window.location.href = '/contact'} className="bg-accent-ink hover:bg-accent-ink text-[hsl(var(--paper))] rounded-full px-8 py-6 h-auto flex items-center gap-3 mt-8">
      ...
      {/* Stats — quiet numeric index */}
      <section className="py-24 md:py-32 bg-card">
        <div className="container mx-auto px-4">
          <span className="micro-label block mb-10">By the numbers</span>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border-t border-border">
            {[
              { v: "15+", t: "Product categories", d: "Rice, wheat, cumin, psyllium and groundnut — a diverse export portfolio." },
              { v: "30+", t: "Buyer connections", d: "Active across Europe, USA, Middle East and Asia, steadily expanding." },
              { v: "100%", t: "Quality commitment", d: "Certified sourcing and ethical trade practice on every consignment." },
              { v: "#1", t: "Private label", d: "Custom packaging and branding support for client-owned labels." },
            ].map((s, i) => (
              <div
                key={s.t}
                className="reveal border-b border-border lg:border-r last:lg:border-r-0 px-0 lg:px-8 py-10 hover-lift"
                style={{ transitionDelay: `${i * 90}ms` }}
              >
                <p className="font-display text-5xl md:text-6xl font-black tracking-tighter text-accent-ink">
                  {s.v}
                </p>
                <p className="micro-label mt-4">{s.t}</p>
                <p className="mt-4 text-sm leading-relaxed text-muted-foreground max-w-[26ch]">{s.d}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16">
            <div>
              <div className="flex items-center gap-2 mb-4 reveal">
                <div className="w-6 h-[2px] bg-accent-ink"></div>
                <span className="text-accent-ink font-bold uppercase tracking-widest text-xs">Core Values</span>
              </div>
              <h2 className="text-5xl font-black text-foreground mb-12 reveal">Our Guiding Principles</h2>
              <div className="space-y-8">
                {principles.map((p) => (
                  <div key={p.id} className="flex gap-6 reveal" style={{ animationDelay: `${p.id * 50}ms` }}>
                    <div className="flex-shrink-0 w-12 h-12 rounded-full border-2 border-border flex items-center justify-center text-accent-ink font-bold text-2xl">
                      {p.id}
                    </div>
                    <div>
      ...
      <section className="py-32 bg-card relative overflow-hidden">
        <div className="container mx-auto px-4">
          <h2 className="text-4xl font-black text-foreground mb-20 reveal">What we offer</h2>
              ...
              {offers.map((offer, idx) => (
                <div key={idx} className={`flex flex-col md:flex-row items-center gap-12 reveal ${offer.pos === 'right' ? 'md:justify-start' : 'md:justify-end'}`}>
                  <div className={`w-full md:w-1/2 flex items-center gap-6 ${offer.pos === 'left' ? 'md:flex-row-reverse md:text-right' : 'md:flex-row md:text-left'}`}>
                    <div className="w-24 h-16 bg-accent-ink rounded-r-3xl rounded-tl-3xl flex items-center justify-center p-4  flex-shrink-0">
                      <CheckCircle2 className="text-[hsl(var(--paper))] w-8 h-8" />
                    </div>
                    <div>
                      <h3 className="text-2xl font-black text-foreground mb-4">{offer.title}</h3>
                      <p className="text-muted-foreground font-medium leading-relaxed">{offer.desc}</p>
                    </div>
                  </div>
                </div>
              ))}
