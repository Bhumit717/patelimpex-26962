...
import cuminExportImage from "@/assets/agricultural-export-processing.jpg";
...
const FAQ = () => {
  ...
  const internalPages = {
    more: [
      ...
      { name: "Groundnut Export", slug: "groundnut-export", description: "Premium groundnut export services" },
      ...
      { name: "Wheat Flour Export", slug: "wheat-flour-export-services", description: "Quality wheat flour export" }
    ],
    seo: [
  ...
  const faqSections = [
    ...
    {
      title: "Cumin Seeds Export from India",
      icon: <FileText className="h-6 w-6" />,
      image: cuminExportImage,
      faqs: [
        {
          question: "Who is the top cumin exporter in India?",
          answer: "India's top cumin exporters are located in Gujarat and Rajkot. Leading exporters specialize in cumin seeds with premium quality standards. Top companies maintain organic certifications, steam sterilization facilities, and global quality standards. Gujarat and Rajkot exporters are particularly strong in cumin exports."
        },
        {
          question: "What are the key export destinations for Indian cumin?",
          answer: "Major export destinations for Indian cumin include USA, UK, UAE, China, and European markets. Export requirements include quality certificates, phytosanitary certification, and compliance with destination country standards. Cumin is one of India's most sought-after agricultural export commodities."
        }
      ]
    },
    {
    ...
    <div className="min-h-screen bg-card">
        ...
        <div className="container mx-auto max-w-6xl relative z-10">
          ...
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-6xl font-black mb-6 text-foreground">
              Frequently Asked <span className="text-accent-ink">Questions</span>
            </h1>
            ...
            {filteredFAQs.map((section, sectionIndex) => (
              <div key={sectionIndex} className="nm-card !p-0 overflow-hidden">
                <div className="relative h-64 overflow-hidden">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t /80 /40 to-transparent"></div>
                  <div className="absolute bottom-6 left-6 text-[hsl(var(--paper))]">
                    <div className="flex items-center mb-2">
                      <div className="p-2 bg-card/20 rounded-lg backdrop-blur-sm mr-3 ">
                        {section.icon}
                      </div>
                      ...
                      return (
                        <div key={faqIndex} className="nm-insert rounded-xl overflow-hidden">
                          <button
                            ...
                            className="w-full px-6 py-4 text-left flex justify-between items-center group bg-card hover:text-accent-ink transition-colors"
                          >
                            <span className="font-semibold text-foreground group-hover:text-accent-ink transition-colors">
                              {faq.question}
                            </span>
                            {openFAQ === globalIndex ? (
                              <ChevronUp className="h-5 w-5 text-accent-ink" />
                            ) : (
                              <ChevronDown className="h-5 w-5 text-muted-foreground group-hover:text-accent-ink transition-colors" />
                            )}
                          </button>
                          {openFAQ === globalIndex && (
                            <div className="px-6 py-4 border-t border-white bg-card/50">
      ...
      <section className="py-16 px-4">
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            <div className="nm-card p-6">
              <div className="flex items-center mb-4">
                <div className="w-10 h-10 rounded-full bg-card shadow-[inset_2px_2px_4px_#cfd6e0,inset_-2px_-2px_4px_#ffffff] flex items-center justify-center mr-3">
                  <MapPin className="h-5 w-5 text-accent-ink" />
                </div>
                <h3 className="text-xl font-bold text-foreground">Market Guides</h3>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                {internalPages.more.slice(0, 8).map((page, index) => (
                  <Link
                    key={index}
                    to={`/more/${page.slug}`}
                    className="text-accent-ink hover:text-accent-ink hover:underline transition-colors p-1"
                  >
                    {page.name}
                  </Link>
                ))}
              ...
              <Link to="/more" className="inline-block mt-6">
                <Button className="nm-btn !px-4 !py-2 !h-auto text-sm text-foreground">
            ...
            <div className="nm-card p-6">
              <div className="flex items-center mb-4">
                <div className="w-10 h-10 rounded-full bg-card shadow-[inset_2px_2px_4px_#cfd6e0,inset_-2px_-2px_4px_#ffffff] flex items-center justify-center mr-3">
                  <Package className="h-5 w-5 text-accent-ink" />
                </div>
                <h3 className="text-xl font-bold text-foreground">Product & Service Pages</h3>
              </div>
              <p className="text-muted-foreground mb-4">
          ...
          {/* Quick Access Links */}
          <div className="nm-card p-8 text-center bg-card">
            <h3 className="text-2xl font-bold mb-4 text-foreground">Need Personalized Assistance?</h3>
            ...
            <div className="flex flex-wrap justify-center gap-6">
              <Link to="/contact">
                <Button className="nm-btn text-accent-ink !w-auto">
