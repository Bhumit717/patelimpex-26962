    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-card">
                ...
                {/* Hero Section */}
                <section className="pt-40 pb-20 bg-card">
                    <div className="container mx-auto px-4">
                        <Link to="/products" className="inline-flex items-center text-muted-foreground hover:text-accent-ink mb-8 font-graduate uppercase text-xs tracking-widest transition-colors">
                            <ArrowLeft className="h-4 w-4 mr-2" />
                            Back to Products
                        </Link>

                        <div className="max-w-4xl">
                            <div className="inline-flex items-center px-4 py-2 bg-card rounded-full border border-border mb-8">
                                <Wheat className="h-4 w-4 text-accent-ink mr-2" />
                                <span className="text-muted-foreground text-[10px] font-bold uppercase tracking-[0.2em] font-graduate">Premium Grains</span>
                            </div>
                            <h1 className="text-5xl md:text-8xl font-black mb-8 text-foreground font-graduate uppercase tracking-tighter">
                                Wheat <span className="text-accent-ink font-fredericka tracking-tight lowercase">Varieties</span>
                            </h1>
                            <p className="text-xl text-muted-foreground font-fondamento italic leading-relaxed mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            {wheatVarieties.map((variety, index) => (
                                <Link
                                    key={index}
                                    to={variety.link}
                                    className="group nm-card !p-0 overflow-hidden hover:-translate-y-3 transition-all duration-500 bg-card block"
                                >
                                    {/* Image Section */}
                                    <div className="relative h-80 overflow-hidden bg-card p-12 flex items-center justify-center">
                                        <img
                                            src={variety.image}
                                            alt={variety.name}
                                    ...
                                    <div className="p-10 border-t border-border">
                                        <h3 className="text-3xl font-black text-foreground mb-4 font-graduate uppercase tracking-tight group-hover:text-accent-ink transition-colors">
                                            {variety.name}
                                        </h3>

                                        <p className="text-muted-foreground font-fondamento italic text-lg mb-6 leading-relaxed">
                                            {variety.description}
                                        </p>
                                        ...
                                        <div className="space-y-3 mb-8">
                                            {variety.specs.map((spec, i) => (
                                                <div key={i} className="flex items-center text-xs font-bold font-graduate uppercase tracking-widest text-muted-foreground">
                                                    <CheckCircle className="h-4 w-4 text-accent-ink mr-3 flex-shrink-0" />
                                                    <span>{spec}</span>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        {/* CTA */}
                                        <div className="flex items-center text-accent-ink font-graduate font-bold uppercase tracking-widest text-xs group-hover:translate-x-2 transition-transform">
