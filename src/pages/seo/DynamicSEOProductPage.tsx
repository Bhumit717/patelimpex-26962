        ...
        <div className="min-h-screen bg-[#e9edf3]">
            ...
            <div className="relative pt-20 overflow-hidden">
                ...
                <div className="relative z-10 container mx-auto px-6 pt-32 pb-24 text-center">
                    ...
                    <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 leading-tight animate-slide-up opacity-0" style={{ animationDelay: '200ms', animationFillMode: 'forwards' }}>
                        Premium <span className="text-orange-500">{product}</span> Export to <span className="text-blue-400">{country}</span>
                    </h1>
                    <p className="text-xl text-slate-300 max-w-3xl mx-auto mb-10 animate-slide-up opacity-0" style={{ animationDelay: '400ms', animationFillMode: 'forwards' }}>
                        Your reliable partner for exporting finest quality {product} to {country}. We ensure international standards, timely delivery, and competitive pricing.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center animate-slide-up opacity-0" style={{ animationDelay: '600ms', animationFillMode: 'forwards' }}>
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        <div className="animate-slide-up opacity-0 stagger-1" style={{ animationFillMode: 'forwards' }}>
                            ...
                            <div className="grid sm:grid-cols-2 gap-6">
                                <div className="flex items-start gap-3 p-4 bg-white/50 rounded-xl shadow-sm hover:shadow-md transition-all">
                                    <div className="p-2 bg-orange-100 rounded-lg text-orange-600 shrink-0"><CheckCircle className="w-5 h-5" /></div>
                                    <div><h3 className="font-semibold text-slate-900 mb-1">Premium Quality</h3><p className="text-sm text-slate-600">Selected best grade</p></div>
                                </div>
                                <div className="flex items-start gap-3 p-4 bg-white/50 rounded-xl shadow-sm hover:shadow-md transition-all">
                                    <div className="p-2 bg-blue-100 rounded-lg text-blue-600 shrink-0"><Globe className="w-5 h-5" /></div>
                                    <div><h3 className="font-semibold text-slate-900 mb-1">{country} Compliant</h3><p className="text-sm text-slate-600">Meets import regulations</p></div>
                                </div>
                                <div className="flex items-start gap-3 p-4 bg-white/50 rounded-xl shadow-sm hover:shadow-md transition-all">
                                    <div className="p-2 bg-green-100 rounded-lg text-green-600 shrink-0"><Truck className="w-5 h-5" /></div>
                                    <div><h3 className="font-semibold text-slate-900 mb-1">Timely Delivery</h3><p className="text-sm text-slate-600">Efficient logistics network</p></div>
                                </div>
                                <div className="flex items-start gap-3 p-4 bg-white/50 rounded-xl shadow-sm hover:shadow-md transition-all">
                                    <div className="p-2 bg-purple-100 rounded-lg text-purple-600 shrink-0"><Package className="w-5 h-5" /></div>
                                    <div><h3 className="font-semibold text-slate-900 mb-1">Export Packaging</h3><p className="text-sm text-slate-600">Secure & customized</p></div>
                                </div>
                            </div>
                        </div>
                        <div className="relative animate-scale-in opacity-0" style={{ animationDelay: '400ms', animationFillMode: 'forwards' }}>
                            <img src="https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&q=80" alt="Patel Impex Export" className="relative rounded-2xl shadow-[15px_15px_30px_#cfd6e0,-15px_-15px_30px_#ffffff] hover:scale-105 transition-transform duration-500 border-4 border-[#e9edf3]" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="py-24 bg-[#e9edf3] reveal">
                <div className="container mx-auto px-6">
                    <h2 className="text-4xl font-bold text-center text-slate-900 mb-16 font-graduate uppercase tracking-tighter">Frequently Asked <span className="text-green-600">Questions</span></h2>
                    <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                        {[
                            { q: `What is the minimum order quantity for ${product} export to ${country}?`, a: `Our minimum order quantity for ${product} to ${country} typically starts from 1 FCL (Full Container Load), but we can arrange smaller trial shipments of 5-10 metric tons depending on the customized packaging requirements for the ${country} market.` },
                            { q: `What documents do you provide for importing ${product} into ${country}?`, a: `We provide a comprehensive set of documents including Commercial Invoice, Packing List, Bill of Lading, Certificate of Origin, Phytosanitary Certificate, and any specific quality test reports required by ${country} customs authorities.` },
                            { q: `What are the shipping times from India to ${country}?`, a: `Transit times to ${country} typically range from 20-35 days, depending on the specific port of discharge and shipping line schedules. We exclusively partner with top-tier logistics providers to ensure timely delivery of your ${product}.` },
                            { q: `Can you provide customized packaging for ${product} in ${country}?`, a: `Yes, we offer fully customizable packaging options for ${product}, including private labeling, branding, and specific bag sizes (25kg, 50kg, retail packs) compliant with ${country}'s labeling and packaging regulations.` }
                        ].map((faq, idx) => (
                            <div key={idx} className={`bg-[#e9edf3] p-8 rounded-3xl shadow-[15px_15px_30px_#cfd6e0,-15px_-15px_30px_#ffffff] hover:shadow-[inset_10px_10px_20px_#cfd6e0,inset_-10px_-10px_20px_#ffffff] transition-all duration-500 group animate-slide-up opacity-0`} style={{ animationDelay: `${idx * 150}ms`, animationFillMode: 'forwards' }}>
                                <h3 className="text-xl font-bold text-slate-800 mb-4 group-hover:text-green-600 transition-colors">{faq.q}</h3>
                                <p className="text-slate-600 leading-relaxed font-fondamento text-lg italic">{faq.a}</p>
                            </div>
                        ))}
                    </div>
                </div>
