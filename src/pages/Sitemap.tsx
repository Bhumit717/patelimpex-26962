...
const Sitemap = () => {
    ...
    return (
        <div className="min-h-screen bg-secondary">
            ...
            {/* Hero */}
            <section className="pt-32 pb-16 px-6 bg-[hsl(var(--ink))] text-[hsl(var(--paper))] relative overflow-hidden">
                ...
                <div className="container mx-auto max-w-7xl relative z-10 text-center">
                    <Badge className="bg-accent-ink mb-6 px-4 py-1 text-xs uppercase tracking-widest font-bold border-none">Navigation Index</Badge>
                ...
                <div className="container mx-auto max-w-7xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                        {sections.map((section, idx) => (
                            <div key={idx} className="bg-card rounded-[2rem]  shadow-slate-200/50 p-8 border border-white hover:border-accent-ink transition-all group overflow-hidden relative">
                                ...
                                <h3 className="text-2xl font-black text-foreground font-graduate mb-8 flex items-center">
                                    {section.title}
                                    <div className="h-0.5 flex-1 bg-secondary ml-4 rounded-full" />
                                </h3>
                                ...
                                <ul className="space-y-4">
                                    {section.links.map((link, lIdx) => {
                                        ...
                                        return (
                                            <li key={lIdx}>
                                                <Link to={link.path} className="flex items-center text-muted-foreground hover:text-accent-ink font-semibold group/link transition-all">
                                                    <div className="w-8 h-8 rounded-lg bg-secondary flex items-center justify-center mr-3 group-hover/link:bg-secondary transition-colors">
                                                        <LinkIcon className="w-4 h-4 text-muted-foreground group-hover/link:text-accent-ink" />
                                                    </div>
                                                    <span className="text-sm md:text-base">{link.name}</span>
                                                    <ArrowRight className="w-4 h-4 ml-auto opacity-0 -translate-x-2 group-hover/link:opacity-100 group-hover/link:translate-x-0 transition-all text-accent-ink" />
                                                </Link>
                                            </li>
                                        );
                                    })}
                                ...
                                {/* Decorative element */}
                                <div className="absolute -bottom-10 -right-10 w-24 h-24 bg-secondary rounded-full group-hover:bg-secondary transition-colors pointer-events-none -z-10" />
                            </div>
                        ))}
                    </div>
