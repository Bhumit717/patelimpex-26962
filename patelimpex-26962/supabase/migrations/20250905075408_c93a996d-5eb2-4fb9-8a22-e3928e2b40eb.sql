...
create policy if not exists "Allow anonymous inserts for analytics" on public.analytics_events
for insert to anon, authenticated using (true) with check (true);

-- Allow anyone to read analytics (UI is password-gated).
create policy if not exists "Allow public read of analytics" on public.analytics_events
